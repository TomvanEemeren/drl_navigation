...
class RosbotEnv(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /scan: Laser Readings
        ...
        super(RosbotEnv, self).__init__(controllers_list=self.controllers_list,
    ...
    def _check_pose_ready(self):
        ...
        while self.global_pose.pose is None and not rospy.is_shutdown():
            try:
                self.global_pose = rospy.wait_for_message(
                    "/rosbot/pose", PoseStamped, timeout=5.0)
                rospy.logdebug("Current /rosbot/pose READY=>")

            except:
                rospy.logerr(
                    "Current /rosbot/pose not ready yet, retrying for getting pose")
